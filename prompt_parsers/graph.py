from .prompts import Form
from .messages import MessagesOfAgent
from .llmapi import LlmApiCalling
from .messages.utils import format_title_message, format_message
...
class Graph:
    def __init__(self):
        ...
        self.human_message, self.system_message, self.bot_message = MessagesOfAgent(
    ...
    def build(self):
        ...
        # Add node
        builder.add_node("create_fullform_NODE", action=self.__create_fullform_NODE)
        builder.add_node("use_gemini_llm_NODE", action=self.__use_gemini_llm_NODE)

        # Add edge
        builder.add_edge(START, "create_fullform_NODE")
        builder.add_edge("create_fullform_NODE", "use_gemini_llm_NODE")
    ...
    def invoke(self, your_prompt: str) -> str:
        if self.is_built:
            ...
            for message_name, message_content in zip(
                self.messages_name, result["messages"]
            ):
            ...
            raise ValueError("Your must build GRAPH first by using 'build' method.")
